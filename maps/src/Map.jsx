...
const Map = (props) => {
    ...
    const drawPointer = (img,pointer) => {
        ...
        current.addEventListener("change.pointer",function(e) {
            ...
            var po = e.coords;
            p = MapPoint.get(po.latitude, po.longitude, debug ? ctx : undefined);
    ...
    const handleClick = (e) => {
        props.onShop(Math.trunc(e.clientX/scale),Math.trunc(e.clientY/scale));
    }

    return (
        <>
            <canvas id="map" ref={view} style={mapStyle} onClick={handleClick}/>
        </>
    )
}
